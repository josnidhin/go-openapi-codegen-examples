...
// All allowed values of TimeUnits enum
var AllowedTimeUnitsEnumValues = []TimeUnits{
	"HOUR",
	"DAY",
	"WEEK",
	"MONTH",
...
func (v *TimeUnits) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := TimeUnits(value)
	for _, existing := range AllowedTimeUnitsEnumValues {
	...
	return fmt.Errorf("%+v is not a valid TimeUnits", value)
}

// NewTimeUnitsFromValue returns a pointer to a valid TimeUnits
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewTimeUnitsFromValue(v string) (*TimeUnits, error) {
	ev := TimeUnits(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for TimeUnits: valid values are %v", v, AllowedTimeUnitsEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v TimeUnits) IsValid() bool {
	for _, existing := range AllowedTimeUnitsEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to TimeUnits value
func (v TimeUnits) Ptr() *TimeUnits {
	return &v
}
