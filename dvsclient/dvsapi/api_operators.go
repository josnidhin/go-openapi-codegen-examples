...
func (a *OperatorsApiService) GetOperatorByIdExecute(r OperatorsApiGetOperatorByIdRequest) (*GetOperators200ResponseInner, *http.Response, error) {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"operator_id"+"}", url.PathEscape(parameterValueToString(r.operatorId, "operatorId")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.operatorId < 1 {
		return localVarReturnValue, nil, reportError("operatorId must be greater than 1")
	}
