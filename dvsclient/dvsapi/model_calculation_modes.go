...
// CalculationModes the model 'CalculationModes'
type CalculationModes string

// List of CalculationModes
const (
	CALCULATIONMODES_SOURCE_AMOUNT      CalculationModes = "SOURCE_AMOUNT"
	CALCULATIONMODES_DESTINATION_AMOUNT CalculationModes = "DESTINATION_AMOUNT"
)

// All allowed values of CalculationModes enum
var AllowedCalculationModesEnumValues = []CalculationModes{
	"SOURCE_AMOUNT",
	"DESTINATION_AMOUNT",
}

func (v *CalculationModes) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := CalculationModes(value)
	for _, existing := range AllowedCalculationModesEnumValues {
	...
	return fmt.Errorf("%+v is not a valid CalculationModes", value)
}

// NewCalculationModesFromValue returns a pointer to a valid CalculationModes
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewCalculationModesFromValue(v string) (*CalculationModes, error) {
	ev := CalculationModes(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for CalculationModes: valid values are %v", v, AllowedCalculationModesEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v CalculationModes) IsValid() bool {
	for _, existing := range AllowedCalculationModesEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to CalculationModes value
func (v CalculationModes) Ptr() *CalculationModes {
	return &v
}

type NullableCalculationModes struct {
	value *CalculationModes
	isSet bool
}

func (v NullableCalculationModes) Get() *CalculationModes {
	return v.value
}

func (v *NullableCalculationModes) Set(val *CalculationModes) {
	v.value = val
	v.isSet = true
}

func (v NullableCalculationModes) IsSet() bool {
	return v.isSet
}

func (v *NullableCalculationModes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCalculationModes(val *CalculationModes) *NullableCalculationModes {
	return &NullableCalculationModes{value: val, isSet: true}
}

func (v NullableCalculationModes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCalculationModes) UnmarshalJSON(src []byte) error {
	v.isSet = true
